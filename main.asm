...
CBLOCK 0x22         ; Start variables at 0x22
    ...
    t1
    tact
    counter_alive ;contador que me mira si cambia de color por alimentacion
    which_table
    rows_printed
...
BUCLE_90s ;llamada a cambiar estado
    ...
    INCF contador_90s, F,ACCESS
    MOVLW .89
    CPFSEQ contador_90s,ACCESS
    RETURN
...
PMW
    ...
    MOVWF t0, ACCESS
    MOVF t1, W, ACCESS
    SUBWF t0, F, ACCESS     ; t0 = 200 - t1

    MOVF t1, W, ACCESS
    BTFSS LATA,1,ACCESS ; si RA1=1 ? usa t1, si no ? t0
    MOVF t0, W,ACCESS

    CPFSLT tact, ACCESS       ; si tact < W ? salta BTG/CLRF
    BTG LATA,1,ACCESS       ; si tact >= W ? toggle
    CPFSLT tact, ACCESS
    CLRF tact, ACCESS
    RETURN

;------------------------------
; CONFIGURAR LA MATRIZ DE LEDS
;------------------------------
...
PRINT_HEALTHY
    ...
    CALL PAINT_GREEN
    BTFSS TABLAT, 0, ACCESS
    CALL PAINT_WHITE

    BTFSC TABLAT, 1, ACCESS
    CALL PAINT_GREEN
    BTFSS TABLAT, 1, ACCESS
    CALL PAINT_WHITE

    BTFSC TABLAT, 2, ACCESS
    CALL PAINT_GREEN
    BTFSS TABLAT, 2, ACCESS
    CALL PAINT_WHITE

    BTFSC TABLAT, 3, ACCESS
    CALL PAINT_GREEN
    BTFSS TABLAT, 3, ACCESS
    CALL PAINT_WHITE

    BTFSC TABLAT, 4, ACCESS
    CALL PAINT_GREEN
    BTFSS TABLAT, 4, ACCESS
    CALL PAINT_WHITE

    BTFSC TABLAT, 5, ACCESS
    CALL PAINT_GREEN
    BTFSS TABLAT, 5, ACCESS
    CALL PAINT_WHITE

    BTFSC TABLAT, 6, ACCESS
    CALL PAINT_GREEN
    BTFSS TABLAT, 6, ACCESS
    CALL PAINT_WHITE

    BTFSC TABLAT, 7, ACCESS
    CALL PAINT_GREEN
    BTFSS TABLAT, 7, ACCESS
    CALL PAINT_WHITE
...
PRINT_CRITICAL
    ...
    RETURN
INIT_TABLE
    MOVWF TBLPTRL,ACCESS      ; Move value from W (0, 16, or 32) into Table Pointer Low
    MOVLW .1           ; Set High Byte to 01 (Base address 0x0100)
